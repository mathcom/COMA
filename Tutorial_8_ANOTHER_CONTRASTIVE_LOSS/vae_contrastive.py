"""
Written by Jonghwan Choi at 29 Sep 2022
https://github.com/mathcom/COMA

The contrastive loss was defined in:
...
class SmilesAutoencoder(nn.Module):
    ...
    def fit(self, dataset,
        ...
        while step < total_steps:
            for batch in DataLoader(dataset, batch_size=batch_size, shuffle=True, drop_last=True, pin_memory=use_cuda):
                ...
                ## fit
                batch_loss = self.partial_fit(*batch_inputs, optim_encoder=optim_encoder, optim_decoder=optim_decoder)
                ...
                loss_contrastive_train = batch_loss[5] # L(src,tar,neg)
                ...
                if (verbose > 0) and (step % display_step == 0):
                    ...
                    log += f"  loss_contrastive(tr): {loss_contrastive_train:.3f}"
    ...
    def partial_fit(self, smiles_A, length_A, smiles_B, length_B, smiles_C, length_C, optim_encoder, optim_decoder):
        '''
        Params
        ------
        ...
        Wang, Yuyang, et al. "Molecular contrastive learning of representations via graph neural networks." Nature Machine Intelligence 4.3 (2022): 279-287.
        '''
        loss_contrastive = self._calc_contrastive_loss(mean_A, mean_B, mean_C)

        ## Total loss
        loss = loss_kld + 0.5 * loss_recon + loss_contrastive
        ...
        return loss.item(), loss_recon_A.item(), loss_recon_B.item(), loss_recon_C.item(), loss_kld.item(), loss_contrastive.item()
    ...
    def _calc_contrastive_loss(self, mean_A, mean_B, mean_C, tau=0.1): # Wang, Yuyang et. al. reported tau=0.1 was the best value of hyperparameter
        norm_cossim_AB = torch.nn.functional.cosine_similarity(mean_A, mean_B) / tau # norm_cossim_AB.shape = (batch, )
        norm_cossim_AC = torch.nn.functional.cosine_similarity(mean_A, mean_C) / tau
        norm_cossim_BC = torch.nn.functional.cosine_similarity(mean_B, mean_C) / tau
        norm_cossim = torch.vstack((norm_cossim_AB, norm_cossim_AC, norm_cossim_BC)) # norm_cossim.shape = (3, batch)
        logsumexp = torch.logsumexp(norm_cossim, dim=0) # logsumexp.shape = (batch, )
        contrastive_loss = torch.mean(logsumexp - norm_cossim_AB, dim=0)
